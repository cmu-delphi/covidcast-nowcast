...
class TestComputeRegressionSensor:
    ...
    def test_compute_regression_sensor_intercept(self, random_normal):
        ...
        assert np.isclose(
            compute_ar_sensor(20200115, values, 1, True, 0),
            -0.09105891 + 0.87530957 * -0.02058535
        )
        assert np.isclose(
            compute_ar_sensor(20200115, values, 2, True, 0),
    ...
    @patch("numpy.random.normal")
    def test_compute_regression_sensor_regularize(self, random_normal):
        ...
        assert np.isclose(compute_ar_sensor(20200115, values, 2, False, 1),
                          -2.8784639 +
